...
type ActionHandler = (state: IPersonsState, actType: any) => IPersonsState;
...
const handlers: HandlersMap = {
    [PersonsActions.SET_DATA]: setData,
};

const INITIAL_STATE: IPersonsState = {
    persons: [{
        "ID": 1,
        "FirstName": "John",
        "LastName": "Heart",
...
export function createPersonsReducer() {
    return function personsReducer(state: IPersonsState = INITIAL_STATE, action): IPersonsState {
...
function setData(state: IPersonsState, action: any) {
    const person = action.person;

    const ds = state.persons.map(item => item.ID === person.ID ? {
        ...item,
        ...person
    } : item);
